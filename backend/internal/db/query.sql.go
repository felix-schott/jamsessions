...
SELECT session_id, session_name, venue, genres, start_time_utc, interval, duration_minutes, description, session_comments, session_website, dt_updated_utc, venue_id, venue_name, address_first_line, address_second_line, city, postcode, geom, venue_website, backline, venue_comments, venue_dt_updated_utc FROM london_jam_sessions.jamsessions s
...
WITH t AS (
    ...
    OR (
        interval = 'Fortnightly' AND EXTRACT(DAY FROM $2 - start_time_utc) % 14 = 0
    )
    OR (
    ...
    OR interval = 'LastOfMonth' AND (
        EXTRACT(MONTH FROM $2 + 7) != EXTRACT(MONTH FROM $2) 
    )
)
SELECT json_build_object(
    'type', 'FeatureCollection',
...
WITH t AS (
    ...
    OR (
        interval = 'Fortnightly' AND EXTRACT(DAY FROM $3 - start_time_utc) % 14 = 0
    )
    OR (
        interval = 'FirstOfMonth' AND CEIL (
    ...
    OR interval = 'LastOfMonth' AND (
        EXTRACT(MONTH FROM $3 + 7) != EXTRACT(MONTH FROM $3) 
    )
)
SELECT json_build_object(
    'type', 'FeatureCollection',
...
WITH t AS (
    ...
    OR (
        interval = 'Fortnightly' AND EXTRACT(DAY FROM $1 - start_time_utc) % 14 = 0
    )
    OR (
        interval = 'FirstOfMonth' AND CEIL (
    ...
    OR interval = 'LastOfMonth' AND (
        EXTRACT(MONTH FROM $1 + 7) != EXTRACT(MONTH FROM $1) 
    )
)
SELECT json_build_object(
    'type', 'FeatureCollection',
...
SET
    ...
    session_comments = coalesce($9, session_comments)
WHERE session_id = $1
`
