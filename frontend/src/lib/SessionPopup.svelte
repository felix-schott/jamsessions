<script lang="ts">
	...
	export let propertiesList: SessionPropertiesWithVenue[];
...
{#if propertiesList.length == 1}
	<Popup {id} title="Click to expand" blur={true} on:click on:close>
		<span slot="title" on:click={() => onClick(propertiesList[0].session_id)}>
			<b>{propertiesList[0].session_name}</b> at <b>{propertiesList[0].venue_name}</b>
		</span>
		<span slot="content">
			<div
				style="display: flex; flex-direction: column;"
				on:click={() => onClick(propertiesList[0].session_id)}
			>
				<div>
...
{:else}
	<Popup {id} title="Click to expand" on:click on:close>
		<span slot="title">
		...
		<span slot="content">
			<div style="display: flex; flex-direction: column;">
				{#each propertiesList as properties, idx}
					<span style="margin-top: 0.4em;" on:click={() => onClick(properties.session_id)}>
						<div>
